...
void INS_Init(void)
{
    ...
    // imu heat init
    TempCtrl.set(PID::ordinary, 1000, 20, 0, 2000, 300, 2000, 2000);
...
void INS_Task(void)
{
    ...
    if ((count % 1) == 0)
    {
        ...
        INS_Gyro[0]=INS.Gyro[Z];
        INS_Gyro[1]=INS.Gyro[X];
        INS_Gyro[2]=INS.Gyro[Y];
...
void IMU_Temperature_Ctrl(void)
{
    ...
    TempCtrl.calc(RefTemp, BMI088.Temperature);
    __HAL_TIM_SET_COMPARE(&htim10, TIM_CHANNEL_1,
                          math::FloatConstrain(math::FloatRounding(TempCtrl.output),
                              0, UINT32_MAX));
}
